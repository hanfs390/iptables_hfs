	       ...
	       struct xt_entry_target *target)
{
	...
	printf("fw.nfcache %d\n", fw->nfcache);
	printf("fw.comefrom %d\n", fw->comefrom);
	printf("fw.target_offset %d\n", fw->target_offset);
	
	size = sizeof(struct ipt_entry);
	for (matchp = matches; matchp; matchp = matchp->next)
		size += matchp->match->m->u.match_size;
	printf("%s, %d: show target***************\n", __func__, __LINE__);
	printf("targetname %s\n", target->u.user.name);
	...
	*e = *fw;
	...
	for (matchp = matches; matchp; matchp = matchp->next) {
		...
		size += matchp->match->m->u.match_size;
	printf("%s, %d: show match***************\n", __func__, __LINE__);
	printf("matchname %s\n", matchp->match->m->u.user.name);
	printf("revision %d\n", matchp->match->m->u.user.revision);
	printf("match_size %d\n", matchp->match->m->u.match_size);
	printf("struct match %d\n", sizeof(struct xt_entry_match));
#if 0
	/* get user-defined match */
	struct xt_udp *m = (struct xt_udp *)(matchp->match->m + 1);
	printf("sport %d-%d\n", m->spts[0], m->spts[1]);
	printf("dport %d-%d\n", m->dpts[0], m->dpts[1]);
	printf("invflags %d\n", m->invflags);
#endif
